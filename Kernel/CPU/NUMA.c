...
static NUMA_runtime_state_t NUMA_state;

static void NUMA_reset(void)
{
    NUMA_state.ready = false;
    NUMA_state.available = false;
    NUMA_state.node_count = 0;
    NUMA_state.mem_range_count = 0;

    for (uint32_t i = 0; i < NUMA_MAX_NODES; i++)
    {
        NUMA_state.node_domain[i] = NUMA_INVALID_NODE;
        for (uint32_t j = 0; j < NUMA_MAX_NODES; j++)
            NUMA_state.distance[i][j] = (i == j) ? NUMA_DISTANCE_LOCAL_DEFAULT : NUMA_DISTANCE_REMOTE_DEFAULT;
    }

    for (uint32_t cpu = 0; cpu < NUMA_CPU_MAP_SIZE; cpu++)
        NUMA_state.cpu_node[cpu] = 0;

    for (uint32_t apic = 0; apic < NUMA_APIC_ID_MAP_SIZE; apic++)
        NUMA_state.apic_node[apic] = 0;
}

static uint32_t NUMA_domain_to_slot(uint32_t domain, bool create)
{
    for (uint32_t i = 0; i < NUMA_state.node_count; i++)
    {
        if (NUMA_state.node_domain[i] == domain)
            return i;
    }

    if (!create || NUMA_state.node_count >= NUMA_MAX_NODES)
        return NUMA_INVALID_NODE;

    uint32_t slot = NUMA_state.node_count++;
    NUMA_state.node_domain[slot] = domain;
    return slot;
}

static uint32_t NUMA_slot_to_domain(uint32_t slot)
{
    if (slot >= NUMA_state.node_count || slot >= NUMA_MAX_NODES)
        return 0;
    return NUMA_state.node_domain[slot];
}
...
static void NUMA_parse_srat(ACPI_SRAT_t* srat)
{
    ...
    while (start + sizeof(ACPI_SRAT_entry_header_t) <= end)
    {
        ...
        switch (entry->type)
        {
            case ACPI_SRAT_TYPE_CPU_AFFINITY:
            {
                ...
                if (cpu_index < NUMA_CPU_MAP_SIZE)
                    NUMA_state.cpu_node[cpu_index] = domain;
                if (apic_id < NUMA_APIC_ID_MAP_SIZE)
                    NUMA_state.apic_node[apic_id] = domain;
            ...
            case ACPI_SRAT_TYPE_MEMORY_AFFINITY:
            {
                ...
                uint32_t slot = NUMA_domain_to_slot(domain, true);
                if (slot == NUMA_INVALID_NODE || NUMA_state.mem_range_count >= NUMA_MAX_MEM_RANGES)
                    break;

                NUMA_memory_range_t* range = &NUMA_state.mem_ranges[NUMA_state.mem_range_count++];
    ...
    if (NUMA_state.node_count > 1)
        NUMA_state.available = true;

    kdebug_printf("[NUMA] SRAT parsed: nodes=%u cpu_aff=%u x2apic_aff=%u mem_aff=%u\n",
                  NUMA_state.node_count,
                  cpu_aff_count,
                  x2apic_aff_count,
                  mem_aff_count);
}
...
static void NUMA_parse_slit(ACPI_SLIT_t* slit)
{
    ...
    for (uint32_t i = 0; i < NUMA_state.node_count; i++)
    {
        ...
        for (uint32_t j = 0; j < NUMA_state.node_count; j++)
        {
            ...
            NUMA_state.distance[i][j] = slit->matrix[idx];
        }
    }
}
...
static void NUMA_log_summary(void)
{
    ...
    uint8_t core_count = APIC_get_core_count();
    for (uint32_t slot = 0; slot < NUMA_state.node_count; slot++)
    {
        ...
        for (uint32_t cpu = 0; cpu < core_count; cpu++)
        {
            if (NUMA_state.cpu_node[cpu] == domain)
                cpu_count++;
        }

        for (uint32_t r = 0; r < NUMA_state.mem_range_count; r++)
        {
            if (NUMA_state.mem_ranges[r].node_id == domain)
                mem_total += NUMA_state.mem_ranges[r].length;
        }
...
{
    return NUMA_state.ready && NUMA_state.available;
}

uint32_t NUMA_get_node_count(void)
{
    if (!NUMA_state.ready || NUMA_state.node_count == 0)
        return 1;
    return NUMA_state.node_count;
}

uint32_t NUMA_get_cpu_node(uint32_t cpu_index)
{
    if (!NUMA_state.ready || cpu_index >= NUMA_CPU_MAP_SIZE)
        return 0;

    return NUMA_state.cpu_node[cpu_index];
}

uint32_t NUMA_get_apic_node(uint32_t apic_id)
{
    if (!NUMA_state.ready || apic_id >= NUMA_APIC_ID_MAP_SIZE)
        return 0;

    return NUMA_state.apic_node[apic_id];
}
...
uint32_t NUMA_get_distance(uint32_t from_node, uint32_t to_node)
{
    ...
    return NUMA_state.distance[from_slot][to_slot];
}

uint32_t NUMA_get_memory_range_count(void)
{
    return NUMA_state.mem_range_count;
}

const NUMA_memory_range_t* NUMA_get_memory_range(uint32_t index)
{
    if (index >= NUMA_state.mem_range_count)
        return NULL;

    return &NUMA_state.mem_ranges[index];
}
